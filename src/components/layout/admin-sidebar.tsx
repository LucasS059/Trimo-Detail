...
export function AdminSidebar() {
  ...
  const handleLogout = async () => {
    try {
      setSaindo(true);
      // Chama a server action que limpa o cookie da sessão
      await encerrarSessao();
      
      // Força a atualização do roteador e manda para o login
      router.push("/login");
      router.refresh();
    } catch (error) {
...
const styles = {
  ...
  linkActive: "flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-zinc-900 bg-zinc-100 transition-colors",
  footer: "p-4 border-t border-zinc-100 shrink-0",
  btnLogout: "w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 transition-colors cursor-pointer disabled:opacity-50",
};